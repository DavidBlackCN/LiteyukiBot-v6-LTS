...
例如 `/help`、`帮助 全部`、`帮助 系统 2`、`帮助 liteyuki_group_manager`、
...
分类来自 `extra.category`：basic/group/utility/minecraft/reminder/
entertainment/system/other，未声明或无效为 other。旧插件因未声明分类
可能集中于其他分类，菜单不维护硬编码插件目录。
...
html_tool → 官方 htmlrender → Playwright → UniMessage 图片发送，无远程背景。
