...
通过可配置的 60s API 获取每日资讯与随机娱乐内容。`sixty_api_base_url` 可替换为自建或其他公共实例；群聊由 `sixty_api_group_mode` 和 `sixty_api_group_ids` 控制，默认白名单。白名单中的群既可响应命令，也是自动推送目标；黑名单模式会从当前 Bot 所在群中排除列表群。
...
- `config.yml` 定义全局默认值和群名单安全边界；当前群 Bot ADMIN 可用 `60s管理` 查看、开关本群功能，并覆盖固定/随机推送设置。群级覆盖不能改变 API、时区、Bot、名单或全局调度参数。
...
- `色图 --source auto|lolicon|random_mage|duckmo|mirlkoi|liemoe|waifuim`、`--size regular|original`、`--no-ai`：选择来源、尺寸或加强 AI 过滤。`auto` 按权重随机选择，只会回退至能够完整表达显式筛选条件、内容分级正确且健康的来源。
